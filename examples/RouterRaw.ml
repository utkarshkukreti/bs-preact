...
module RouterRaw = struct
  let make =
   fun [@preact.component "RouterRaw"] () ->
    ...
    let push string _ = P.Router.Url.push (P.Router.Url.fromString string) in
    let replace string _ = P.Router.Url.replace (P.Router.Url.fromString string) in
    P.div
      ...
      ; P.h3 [] [ P.string (P.Router.Url.toString url) ]
      ]
end
